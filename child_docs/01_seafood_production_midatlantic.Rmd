...
```{r child-header, eval = params$id_child_docs}
paste("Beginning", knitr::current_input())
```
...
```{r climatevul-land, fig.width = 6.5, fig.asp = 0.5, fig.cap = return_caption(chunk_name = "climatevul-land")}
...
```{r, rec-landings, fig.cap = return_caption(chunk_name = "rec-landings")}
rec_landings_plot
```
...
```{r rec-hms, fig.cap=return_caption(chunk_name = "rec-hms"), fig.width=6.5, fig.asp=.35}
rec_hms_plot
```
...
```{r stock-status, fig.width = 6.5,  fig.asp = 0.5, fig.cap = return_caption(chunk_name = "stock-status")}
stock_status_plot$p +
...
```{r abcacl-stacked, fig.cap=return_caption(chunk_name = "abcacl-stacked"), fig.width = 6, fig.asp = 0.4, fig.height = 4}
abcacl_stacked_plot
```
...
```{r abcacl-catch, fig.cap = return_caption(chunk_name = "abcacl-catch")}
abcacl_catch_plot
```
...
```{r nefsc-biomass-mab, fig.cap =  return_caption(chunk_name = "nefsc-biomass-mab"), fig.width=6.5, fig.asp = 0.75, results='hide'}
ecodata::plot_aggregate_biomass(report = params$region, EPU = "MAB", n = 10)
```
...
```{r child-footer, eval = params$id_child_docs}
