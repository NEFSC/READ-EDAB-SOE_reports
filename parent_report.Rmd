...
caption_key <- read.csv(params$fig_caption) |>
  dplyr::mutate(caption = dplyr::case_when(
    params$region == "MidAtlantic" ~ .data$fig_cap_ma,
    params$region == "NewEngland" ~ .data$fig_cap_ne
  ),
  filepath = dplyr::case_when(
    params$region == "MidAtlantic" ~ .data$fig_path_ma,
    params$region == "NewEngland" ~ .data$fig_path_ne
  )) |>
  dplyr::mutate(caption = dplyr::case_when(is.na(caption) ~ .data$fig_cap_both,
                                           TRUE ~ caption),
                filepath = dplyr::case_when(is.na(filepath) ~ .data$fig_path_both,
                                           TRUE ~ filepath) |>
                  stringr::str_replace_all("\\\\", "/")) 
## TODO: the single filepath and caption colums can be deprecated (removed)
...
```{r}
child_file <- here::here(paste0("child_docs/01_seafood_production_", stringr::str_to_lower(params$region), ".Rmd"))
...
```{r}
child_file <- here::here(paste0("child_docs/02_commercial_profits_", stringr::str_to_lower(params$region), ".Rmd"))
...
return_plot("NARW-calf-abundance")
```
...
```{r calfin-cog, fig.cap = return_caption(chunk_name = "calfin-cog"), fig.width = 7.5, fig.asp = .3, eval = params$region == 'NewEngland'}
return_plot("calfin-cog")
```

```{r forageshifts, fig.cap=return_caption(chunk_name = "forageshifts"), fig.width = 6.5, fig.asp = .3}
return_plot("forageshifts")
```
...
return_plot("macrobenthosshifts")
```
...
```{r wind-proposed-dev, fig.cap = return_caption(chunk_name = "wind-proposed-dev"), fig.cap=txt, fig.width=5}
return_plot("wind-proposed-dev")
```
...
```{r slopewater, fig.asp=.5, fig.cap=return_caption(chunk_name = "slopewater")}
return_plot("slopewater")
```
...
```{r cocobloom, out.width="55%", fig.cap=return_caption(chunk_name = "cocobloom")}
return_plot("cocobloom")
```
...
```{r GOMoa, out.width="60%", fig.cap=return_caption(chunk_name = "GOMoa")}
return_plot("GOMoa")
```
...
```{r docformat, fig.cap = return_caption(chunk_name = "docformat"),  fig.width = 6.5, fig.height = 2.5}
