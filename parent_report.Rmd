...
caption_key <- read.csv(params$fig_caption) |>
  ...
  filepath = dplyr::case_when(
    params$region == "MidAtlantic" ~ fig_path_ma,
    params$region == "NewEngland" ~ fig_path_ne
  )) |>
  dplyr::mutate(caption = dplyr::case_when(is.na(caption) ~ fig_cap_both,
                                           TRUE ~ caption),
                filepath = dplyr::case_when(is.na(filepath) ~ fig_path_both,
                                           TRUE ~ filepath))
...
```{r}
## TODO: insert calanus COG plot for NE only
```
...
knitr::include_graphics(wind_cumul_image)
```
...
magick::image_read(whales_wind_image)
```
...
magick::image_read(slopesea_image) |>
...
magick::image_read(cocobloom_image) |>
...
magick::image_read_pdf(GOMoa_image) |>
