...
TEX_FILE <- here::here("SOE2026_MAFMC_SOE.tex")
...
get_tex_status <- function(tex_path) {
  if (!file.exists(tex_path)) {
    stop("TeX file not found. Render PDF with 'keep_tex: true' first.")
  }
      ...
      recommendation = {
        ...
        if (nrow(chunk_match) > 0) {
          rec <- paste0(
            ...
            clean_key,
            "' in ",
            basename(chunk_match$file),
            " (Line ",
            chunk_match$line,
            "). RMarkdown adds 'fig:' automatically. ",
          ...
          if (length(best_idx) > 0 && dists[best_idx] <= 3) {
            # Threshold for typo
            match_val <- all_defs[best_idx]
            match_meta <- rmd_data_df %>%
              filter(name == match_val, type != "usage") %>%
              slice(1)
            rec <- paste0(
              "TYPO? Did you mean '",
              match_val,
              "'? Found in ",
              basename(match_meta$file),
              " (Line ",
              match_meta$line,
              ")"
            )
...
tryCatch(
  {
    ...
    if (nrow(broken) == 0) {
      ...
      message(paste(
        "Found",
        nrow(broken),
        "broken references. Scanning source Rmds..."
      ))
