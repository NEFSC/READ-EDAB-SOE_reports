...
extract_urls_from_tex <- function(file_path) {
  ...
  extracted_links <- df_text %>%
    ...
    mutate(
      ...
      matches_href = list(str_match_all(
        text,
        "\\\\href\\{([^}]+)\\}\\{([^}]*)\\}"
      )[[1]]),
  ...
  href_df <- extracted_links %>%
    ...
    mutate(
      matches_href = map(matches_href, function(m) {
  ...
  url_cmd_df <- extracted_links %>%
    ...
    mutate(
      matches_url = map(matches_url, function(m) {
...
if (nrow(links_df) == 0) {
  stop(
    "No links found. Check if the .tex file is empty or formatted unexpectedly."
  )
}
...
output_filename <- paste0(
  basename(tools::file_path_sans_ext(TEX_FILE)),
  "_url_check_",
...
print("--- Issues Found ---")
print(
  final_report %>% filter(!verified) %>% select(section_name, url, status_code)
)
